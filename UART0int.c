...
//#include "../RTOS_Labs_common/FIFOsimple.h" // mj uncommented
...
Sema4Type UARTTXFree;
Sema4Type UARTRXFree;

void UART_TXConsumer(){
  while (true){
    char c = OS_Fifo_Get(&TXFifo);
...
void UART_Init(void){
  ...
  OS_Fifo_Init(&RXFifo, 256);                        // initialize empty FIFOs
  OS_Fifo_Init(&TXFifo, 256);
  OS_InitSemaphore(&UARTTXFree, 1);
  OS_InitSemaphore(&UARTRXFree, 1); // mj commented this plus 3 above
	
	//RxFifo_Init(); // mj added
	//TxFifo_Init(); // mj added
  ...
  GPIO_PORTA_AMSEL_R = 0;               // disable analog functionality on PA
                                        // UART0=priority 2
  NVIC_PRI1_R = (NVIC_PRI1_R&0xFFFF00FF)|0x00000000; // bits 13-15
  NVIC_EN0_R = NVIC_EN0_INT5;           // enable interrupt 5 in NVIC
  
  //OS_AddThread(&UART_TXConsumer, 1024, 0);
}
...
void UART_OutTime(uint64_t time){
  // hours
  uint32_t hours = time / 80000 / 1000 / 60 / 60;
  uint32_t minutes = time / 80000 / 1000 / 60 % 60;
  uint32_t seconds = time / 80000 / 1000 % 60;
