...
#define left 0x10101010
#define right 0x20202020
...
void ST7735_Message(uint32_t  d, uint32_t  l, char *pt, int32_t value){
  ...
  OS_bWait(&LCDFree);
	...
	if(l > 7 && l < 0) return; //wrong bound of line input
	
	if(d == 0){ //output to the top section
		if(pt != NULL) ST7735_DrawString(0,l,pt,ST7735_YELLOW); //print string to the LCD
		...
		if(value == left) ST7735_DrawString(14,l,"Left",ST7735_YELLOW);
		else if(value == right) ST7735_DrawString(14,l,"Right",ST7735_YELLOW);
    else ST7735_OutUDec2(value, l);
  }
  else if(d == 1){ // output from the bottom section
    if(pt != NULL) ST7735_DrawString(0,8+l,pt,ST7735_YELLOW); //print string if not null
		if(value < 0) {
			ST7735_OutChar('-');
			value &=0xEFFF;
		}
		if(value == left) ST7735_DrawString(14,8+l,"Left",ST7735_YELLOW);
		else if(value == right) ST7735_DrawString(14,8+l,"Right",ST7735_YELLOW);
    else ST7735_OutUDec2(value, 8+l);
  }
	//EndCritical(tmp);
  OS_bSignal(&LCDFree);
