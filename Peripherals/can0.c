...
uint32_t RCV_ID=4; // set dynamically at time of CAN0_Open
uint32_t XMT_ID=2; 
...
void CAN0_SendData(uint8_t* data, uint32_t messageID, uint32_t bytes){
// in this case there is just one type, but you could accept multiple ID types
  CAN0_Setup_Message_Object(messageID, NULL, bytes, data, messageID, MSG_OBJ_TYPE_TX);
}
