    ...
    case 4:   // filter out data and put it into the right receive FIFO
      if(ReceiveDataLen) {
        switch(ReceiveDataStream) {
          case 0: if(OS_Fifo_Put(&ESP8266Rx0, letter) == false){  // overflow, data loss
...
void ESP8266_InitUART(int rx_echo, int tx_echo){ volatile int delay;
  OS_Fifo_Init(&ESP8266Rx, 256);
  OS_Fifo_Init(&ESP8266Rx0, 256);
  ESP8266_EchoResponse = rx_echo;
  ESP8266_EchoCommand = tx_echo;
#if ESP8266_UART==1
...
// UART_ESP8266(_FBRD_R) = 26;   // FBRD = round(0.4028 * 64 ) = 26
  ...
  UART_ESP8266(_IFLS_R) += (UART_IFLS_RX1_8);// RX and TX FIFO interrupt threshold >= 1/8th full
  UART_ESP8266(_IM_R)  |= (UART_IM_RXIM|UART_IM_RTIM); // Enable interupt on TX, RX and RX transmission end
  UART_ESP8266(_CTL_R) |= (UART_CTL_UARTEN|UART_CTL_RXE); // Set UART enable bit 
}
...
void ESP8266_OutChar(char data){
  ...
  if(ESP8266_EchoCommand){
    UART_OutCharNonBlock(data); // echo
  }
  UART_ESP8266(_DR_R) = data;
  EndCritical(tmp);
}
...
int ESP8266_Init(int rx_echo, int tx_echo){ char c; const char* s; uint32_t timer = 1;
  ...
  timer = 10000000;  // around a 1-2s total timeout
    s = ESP8266_READY_RESPONSE;
  while(*s) {
    while(timer && ((UART_ESP8266(_FR_R)&UART_FR_RXFE) != 0)) { timer--; }
    if(!timer) break;  
    c = (char)(UART_ESP8266(_DR_R)&0xFF);
    if(rx_echo) UART_OutChar(c); // echo, requires UART0 to be operational, will print garbage
    if(c == *s) {
      s++;
    } else {
      s = ESP8266_READY_RESPONSE;
...
// buffer for UART like output
char TXBufferBuffer[129];
int bufferLen = 0;
...
int ESP8266_FlushBufferBuffer(){
	int size = strlen(TXBufferBuffer);
	if (size > 0){
		char TXBuffer[32];
		if(ESP8266_ConnectionMux) {
			sprintf(TXBuffer, "AT+CIPSEND=%d,%d\r\n", 0, size);
		...
		ESP8266_SendCommand(TXBufferBuffer);
		memset(TXBufferBuffer, 0, 129);
		bufferLen = 0;
...
void ESP8266_BufferedChar(char c){
	if (bufferLen == 128){
		// flush buffer
		ESP8266_FlushBufferBuffer();
		bufferLen = 0;
	}
	TXBufferBuffer[bufferLen++] = c;
}

void ESP8266_BufferedUDec(int n){
	if(n >= 10){
    ESP8266_BufferedUDec(n/10);
    n = n%10;
  }
  ESP8266_BufferedChar(n+'0'); /* n is between 0 and 9 */
}

void ESP8266_BufferedString(char* str){
	while (*str != 0){
		ESP8266_BufferedChar(*(str++));
	}
}
...
int ESP8266_Receive(char* fetch, uint32_t max){ long sr; const char* s;
  char letter;
  while(max > 1) {
    if(OS_Fifo_Size(&ESP8266Rx0) || ESP8266_DataAvailable) { // data (about to be) available?
      letter = OS_Fifo_Get(&ESP8266Rx0);
...
int ESP8266_CloseTCPConnection(void){ int try=MAXTRY;
  while(try){
    ...
    if(ESP8266_WaitForResponse(ESP8266_OK_RESPONSE,ESP8266_ERROR_RESPONSE)) {
      OS_Fifo_Reset(&ESP8266Rx0);
      return SUCCESS;
    }
    try--;
  }
  OS_Fifo_Reset(&ESP8266Rx0);  // Discard any data
  return FAILURE;
}
